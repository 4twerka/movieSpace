...
function Series() {
  ...
  const renderSection = (title, items, keyName) => {
    const isExpanded = expandedSections[keyName];
    const itemsToRender = isExpanded ? items : items.slice(0, 12);
      ...
      <div className="mb-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {itemsToRender.map((item) => (
            <Link to={`/seriesPage/${item.id}`} key={item.id}>
