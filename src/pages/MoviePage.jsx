      ...
      {isLoading ? (<div className="flex justify-center text-5xl text-white items-center">Loading...</div>) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="space-y-3 text-gray-400">
            <p>
              <span className="text-white font-semibold">Genres:</span> {movie.genres?.map((g) => g.name).join(", ")}
            </p>
            <p>
              <span className="text-white font-semibold">IMDB Rating:</span> ⭐ {parseInt(movie.vote_average)}/10
            </p>
            <p>
              <span className="text-white font-semibold">Box Office:</span> ${movie.revenue.toLocaleString()}
            </p>
          </div>
        </div>
      </div>
